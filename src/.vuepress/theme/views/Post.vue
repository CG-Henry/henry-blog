<template>
  <div>
    <div class="page page__full row justify-right">
      <article class="column xs-100 page__full__box box-default"
               itemscope
               itemprop="blogPost"
               itemtype="https://schema.org/BlogPosting">
        <meta itemprop="mainEntityOfPage"
              :content="currentPost.path">

        <header class="page-header page__full-header">
          <back-button />
          <div class="page-header__meta">
            <time-provider type="ago"
            ...
            <meta itemprop="dateModified"
                  content="currentPost.updated_at">

            <time-provider type="mintoread"
          ...
          <h1 itemprop="name headline"
              class="page-header__title">{{ currentPost.title }}</h1>
          <div class="page-header-cat">
            <nav class="page-header-cat__nav">
              <ul class="page-header-cat__list">
                <li class="page-header-cat__item"
                    v-for="category in getCategories"
                    :key="category.key">
                  <router-link class="page-header-cat__link meta-text"
                               itemprop="keywords"
                               :to="category.path">
          ...
          <div v-if="getAuthor"
            ...
            <router-link class="link link--filler-s-primary"
                         rel="author"
                         itemprop="url"
                         :to="getAuthor.path">
        ...
        <div v-if="currentPost.cover && !currentPost.video"
             class="post-cover"
             itemprop="image"
             itemscope
             itemtype="https://schema.org/ImageObject">
          <responsive-picture :coverName="currentPost.coverName">
            <img class="post-cover__image cover"
                 itemprop="url"
                 :src="`/blog${currentPost.coverName}.${currentPost.coverExt || $themeConfig.responsive.ext || 'png'}`"
                 :title="currentPost.coverAlt"
                 :alt="currentPost.coverAlt">
          </responsive-picture>
        </div>

        <section class="post-cover post-cover--video"
                 v-if="currentPost.video">
          <lazy-load tag="iframe"
                     :data="{ src: currentPost.video, height: '100%' }" />
        </section>
    ...
    <section v-if="$themeLocaleConfig.newsletter.action"
    ...
    <aside class="container main">
      <!-- <more-posts label="related_posts"
                  :posts="relatedPosts">
...
export default {
  ...
  components: {
    ...
    TableContents: () =>
      import(
        /* webpackChunkName = TableContents" */ "@theme/components/TableContents"
      ),
    KtButton: () =>
    ...
    VueDisqus: () =>
      import(
        /* webpackChunkName = "vue-disqus" */ "vue-disqus/dist/vue-disqus.vue"
      ),
    BackButton: () =>
      import(
        /* webpackChunkName = "BackButton" */ "@theme/components/BackButton"
      ),
    Newsletter: () =>
      import(
        /* webpackChunkName = "Newsletter" */ "@theme/components/Newsletter"
      ),
    LazyLoad: () =>
