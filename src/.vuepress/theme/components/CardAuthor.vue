<template>
  <div class="card-author box-default"
       :class="{ 'card-author--no-shadow': !shadow }"
       itemscope
       itemtype="https://schema.org/Person">
    <div class="row">
      <div class="column xs-33">
        <img class="card-author__avatar"
      ...
      <div class="column xs-67 card-author-info">
        <div class="card-author-info__role">
          ...
          <span itemprop="jobTitle"
                class="meta-text">{{ author.frontmatter.role }}</span>
        </div>

        <router-link itemprop="url"
                     class="card-author__link"
                     :to="`${$localeConfig.path}${$t('path_route_authors')}/${author.frontmatter.nickname}.html`">
          <h3 itemprop="name"
              class="card-author-info__title">{{ author.frontmatter.name }}</h3>
        </router-link>

        <div class="card-author-info__joined">
          <span class="icon">enter</span>
          <time-provider type="ago"
                         :date="author.frontmatter.created_at">
            <span class="text"
                  slot-scope="{ time }">{{ $t('joined') }} 3 年</span>
          </time-provider>
          ...
          <ul class="card-author-social__list">
            <li class="card-author-social__item"
              ...
              <a itemprop="sameAs"
                 ...
                 target="_blank">
                <span class="icon card-author-social__icon"
                      :class="`icon-${network.name}`">{{ network.name }}</span>
              </a>
            </li>
          </ul>
        </div>
